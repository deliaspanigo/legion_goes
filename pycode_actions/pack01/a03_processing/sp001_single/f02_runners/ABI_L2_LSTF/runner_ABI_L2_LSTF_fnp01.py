"""
Path: legion_goes/pycode_actions/pack01/a03_processing/sp001_single/f02_runners/ABI_L2_LSTF/runner_ABI_L2_LSTF.py
Version: 0.0.2
Description: Runner - Land Surface Temperature with Universal Path Detection.
Last modification: 05-05-2026 18:18
"""
# ========================================================================================================================================
#  Execution: python3 -m  legion_goes.pycode_actions.pack01.a03_processing.sp001_single.f02_runners.ABI_L2_LSTF.runner_ABI_L2_LSTF_fnp01
# ========================================================================================================================================
...
def run_runner_ABI_L2_LSTF_fnp01(nc_path):
    """
    ...
    # 2. Check for existing valid outputs (Skip-logic)
    if is_processing_complete(dict_path_output):
        print(f"  [SKIPPED] {nc_path.name} (Outputs Check OK)")
        return

    # 3. Clean up partial or empty files before reprocessing
    for p in dict_path_output.values():
        if p.exists(): p.unlink()

    print(f"  [PROCESSING] {nc_path.name}...")
    
    # 4. Execute core processing
...
if __name__ == "__main__":
    ...
    # 2. Look for the first .nc file containing 'LSTF'
    ...
    if not nc_candidates:
        ...
        # Start runner
        run_runner_ABI_L2_LSTF_fnp01(nc_path=target_nc)
        print("-" * 80 + "\n✅ TEST COMPLETED\n" + "=" * 80)
