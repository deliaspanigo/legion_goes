"""
Path: legion_goes/pycode_actions/pack01/a03_processing/sp001_single/f02_runners/ABI_L2_LSTF/runner_ABI_L2_LSTF_fnp02.py
Version: 0.0.1
Description: Runner - Land Surface Temperature FNP02 with explicit data_proc path and final output validation.
Last modification: 21-05-2026
"""

# ========================================================================================================================================
#  Execution:
#  python -m legion_goes.pycode_actions.pack01.a03_processing.sp001_single.f02_runners.ABI_L2_LSTF.runner_ABI_L2_LSTF_fnp02
# ========================================================================================================================================
...
from legion_goes.pycode_actions.pack01.a03_processing.sp001_single.f01_code.ABI_L2_LSTF.proc_ABI_L2_LSTF_fnp02 import (
    gen_dict_output_file_name,
    run_proc_ABI_L2_LSTF_fnp02,
)
...
def gen_dict_path_output(nc_path, str_folder_path_data_proc=None):
    ...
    if data_proc_dir.exists() and not data_proc_dir.is_dir():
        raise ValueError(
            f"str_folder_path_data_proc exists but is not a folder: {data_proc_dir}"
        )
...
def get_missing_or_empty_outputs(output_dict: dict) -> dict:
    """
    Returns a dictionary with outputs that do not exist or are empty.
    """
...
def is_processing_complete(output_dict: dict) -> bool:
    ...
    Returns True only if EVERY expected file is valid.
    """
...
def count_valid_outputs(output_dict: dict) -> int:
    """
    Counts files that exist, are files, and are not empty.
    """
...
def run_runner_ABI_L2_LSTF_fnp02(nc_path, str_folder_path_data_proc=None, overwrite=False):
    """
    Main runner logic for FNP02: manages skip-logic, cleanup, core execution,
    and final validation of expected outputs.

    Returns
    -------
    bool
        True if processing is complete or skipped because outputs already exist.
        False if the core fails or expected outputs are missing/empty after processing.
    """
    ...
    # Clean only expected output files before reprocessing.
...
if __name__ == "__main__":
    ...
    print("\n" + " Runner LSTF FNP02: DIAGNOSTIC TEST ".center(80, "="))
