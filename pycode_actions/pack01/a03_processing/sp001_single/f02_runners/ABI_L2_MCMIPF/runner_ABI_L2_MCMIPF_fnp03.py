"""
Path:
legion_goes/pycode_actions/pack01/a03_processing/sp001_single/f02_runners/ABI_L2_MCMIPF/runner_ABI_L2_MCMIPF_fnp03.py

Description:
    Runner for ABI-L2-MCMIPF FNP03.
"""
...
from legion_goes.pycode_actions.pack01.a03_processing.sp001_single.f01_code.ABI_L2_MCMIPF.proc_ABI_L2_MCMIPF_fnp03 import (
    gen_dict_output_file_name,
    run_proc_ABI_L2_MCMIPF_fnp03,
)
...
def run_runner_ABI_L2_MCMIPF_fnp03(nc_path, str_folder_path_data_proc=None, overwrite=False):
    nc_path = Path(nc_path)
    ...
    exists_count = sum(1 for p in dict_path_output.values() if p.exists() and p.stat().st_size > 0)
    ...
    try:
        with silence_runner_noise():
            success = run_proc_ABI_L2_MCMIPF_fnp03(
                nc_path=str(nc_path),
                **dict_str_paths,
            )
...
if __name__ == "__main__":
    print("\n" + " FNP03: MCMIPF RUNNER TEST ".center(80, "="))
    ...
    if not nc_candidates:
        ...
        run_runner_ABI_L2_MCMIPF_fnp03(
            ...
            str_folder_path_data_proc=working_dir / "data_proc_test",
            overwrite=True,
        )
