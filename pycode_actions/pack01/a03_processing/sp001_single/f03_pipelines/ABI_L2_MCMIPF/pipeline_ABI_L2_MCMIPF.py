"""
Path:
legion_goes/pycode_actions/pack01/a03_processing/sp001_single/f03_pipelines/ABI_L2_MCMIPF/pipeline_ABI_L2_MCMIPF.py

Version: 0.0.2
Description:
    Pipeline/orquestador para ABI-L2-MCMIPF.

    Este pipeline:
    - Recibe un nc_path y una folder data_proc.
    - Crea una folder _control al same nivel que las folders fnp01, fnp02, etc.
    - Runs una o varias FNPs.
    - Por ahour soporta fnp01.
    - Writes clean logs for each run.
    - Generates manifest.json so R/Shiny can read what happened.
...
# =========================================================================================================================================
#  Execution:
#  python -m legion_goes.pycode_actions.pack01.a03_processing.sp001_single.f03_pipelines.ABI_L2_MCMIPF.pipeline_ABI_L2_MCMIPF
# =========================================================================================================================================
...
from legion_goes.pycode_actions.pack01.a03_processing.sp001_single.f02_runners.ABI_L2_MCMIPF.runner_ABI_L2_MCMIPF_fnp01 import (
    gen_dict_path_output as gen_dict_path_output_fnp01,
    run_runner_ABI_L2_MCMIPF_fnp01,
)

from legion_goes.pycode_actions.pack01.a03_processing.sp001_single.f02_runners.ABI_L2_MCMIPF.runner_ABI_L2_MCMIPF_fnp02 import (
    gen_dict_path_output as gen_dict_path_output_fnp02,
...
def parse_goes_nc_file(nc_path) -> Dict[str, str]:
    ...
    Example:
    OR_ABI-L2-MCMIPF-M6_G19_s20260031200230_e20260031209539_c20260031214569.nc
    """
...
def gen_timestamp_folder(nc_path, data_proc_dir) -> Path:
    ...
    data_proc/sp01_single/noaa-goes19-EAST/ABI-L2-MCMIPF/YYYY/DDD/HH/sYYYYDDDHHMMSS
    """
...
def run_fnp01_for_pipeline(nc_path, data_proc_dir, overwrite=False) -> Dict[str, Any]:
    ...
    try:
        success = run_runner_ABI_L2_MCMIPF_fnp01(
            nc_path=nc_path,
            str_folder_path_data_proc=data_proc_dir,
            overwrite=overwrite,
        )
...
def run_fnp02_for_pipeline(nc_path, data_proc_dir, overwrite=False) -> Dict[str, Any]:
    """
    Wrapper around runner FNP02.
    ...
    dict_path_output = gen_dict_path_output_fnp02(
        nc_path=nc_path,
        str_folder_path_data_proc=data_proc_dir,
    )
    ...
    try:
        success = run_runner_ABI_L2_MCMIPF_fnp02(
            nc_path=nc_path,
            str_folder_path_data_proc=data_proc_dir,
            overwrite=overwrite,
        )
    ...
    if success and len(missing_after) == 0:
        ...
        if error is None:
            error = (
                "Runner returned False. "
                "Check fnp02.log for the real processing error. "
                f"Missing outputs after processing: {missing_after}"
            )
...
def run_pipeline_ABI_L2_MCMIPF(
    nc_path,
    data_proc_dir,
    overwrite=False,
    fnps=None,
    ...
    """
    Orchestrates one or more MCMIPF FNPs.
...
if __name__ == "__main__":
    ...
    print("\n" + " MCMIPF PIPELINE: DIAGNOSTIC TEST ".center(80, "="))
    ...
    # -------------------------------------------------------------------------
    # 2. Look recursively for the first .nc file containing 'MCMIPF'
    # -------------------------------------------------------------------------

    nc_candidates = sorted(list(folder_data_raw.rglob("*MCMIPF*.nc")))

    if not nc_candidates:

        print(f" Error: No .nc files with 'MCMIPF' found in: {folder_data_raw}")
        ...
        manifest = run_pipeline_ABI_L2_MCMIPF(
            nc_path=target_nc,
