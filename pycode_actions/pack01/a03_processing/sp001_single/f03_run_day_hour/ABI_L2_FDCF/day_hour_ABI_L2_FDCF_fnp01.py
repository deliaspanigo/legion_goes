"""
Path: legion_goes/pycode_actions/pack01/a03_processing/sp001_single/f03_run_day_hour/ABI_L2_FDCF/day_hour_ABI_L2_FDCF_fnp01.py
...
"""
# ================================================================================================================================================
#  Execution: python3 -m  legion_goes.pycode_actions.pack01.a03_processing.sp001_single.f03_run_day_hour.ABI_L2_FDCF.day_hour_ABI_L2_FDCF_fnp01
# ================================================================================================================================================
...
from legion_goes.pycode_actions.pack01.a03_processing.sp001_single.f02_runners.ABI_L2_FDCF.runner_ABI_L2_FDCF_fnp01 import run_runner_ABI_L2_FDCF_fnp01

def run_day_hour_ABI_L2_FDCF_fnp01(position: str, year: str, day: str, hour: str, overwrite = False):
    """
    Processes all FDCF files for a specific day/hour or an entire day.
    """
    # 1. Identify Satellite and Product
    sat_id = get_sat_id_by_date(position=position, year=year, day=day)
    product_id = "ABI-L2-FDCF" 
    ...
    for h in hours_to_process:
        ...
        nc_candidates = sorted(list(selected_folder_raw.rglob("*FDCF*.nc")))
        ...
        for nc_file in nc_candidates:
            ...
            try:
                run_runner_ABI_L2_FDCF_fnp01(nc_path=str(nc_file), overwrite = overwrite)
...
if __name__ == "__main__":
    print("\n" + " Run  Day-Hour FNP01: FDCF RUNNER (MULTI-HOUR) ".center(80, "="))
    
    try:
        # Configuration for processing
        # Use "ALL" to process all 24 hour-folders or a specific hour string (e.g., "12")
        run_day_hour_ABI_L2_FDCF_fnp01(
            position = "EAST", 
            year     = "2026", 
            day      = "003", 
            hour     = "ALL"
