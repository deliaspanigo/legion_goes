"""
FDCF FNP01 reference metadata.

This module is the new product-level source of truth for ABI-L2-FDCF Mask
categories, official meanings, and Satpy visual colors.
"""
...
FDCF_PUG_REFERENCE = (
    ...
    "Level 2+ Products; Fire/Hot Spot Characterization; Fire Mask Flag "
    "Values and Meanings."
)
...
def fdcf_official_mask_categories() -> List[Dict[str, object]]:
    """Return the full official ABI-L2-FDCF Mask category table."""
...
def _abi_enhancement_yaml() -> Path:
    legion_goes_root = Path(__file__).resolve().parents[2]
    return legion_goes_root / "satpy_config" / "enhancements" / "abi.yaml"
...
def fdcf_palette_from_abi_yaml(
    variant: str = "color01",
    yaml_path: Optional[str] = None,
) -> Dict[int, Dict[str, object]]:
    """
    Read the Satpy ABI enhancement YAML and return the color table for a variant.
    """
    ...
    for line in text:
        ...
        r, g, b, a, code, label = match.groups()
        code_i = int(code)
        variant_number = int(re.search(r"([0-9]+)$", enhancement).group(1))
        palette[code_i] = {
            "r": int(r),
            "g": int(g),
            "b": int(b),
            ...
            "variant": f"color{variant_number:02d}",
        }
...
def fdcf_mask_counts(nc_path: Optional[str] = None) -> Dict[int, int]:
    """Count FDCF Mask classes in a NetCDF file."""
...
def fdcf_reference_table(
    ...
    """
    Return the full or active-only reference table for one FDCF visual variant.
    """
    ...
    for row in fdcf_official_mask_categories():
        ...
        if active_only and not visible_in_selected_png:
            continue
        ...
        out.update(
            {
                ...
                "enhancement": color_info.get(
                    "enhancement",
                    _variant_to_enhancement(variant),
                ),
                "r": color_info.get("r"),
                "g": color_info.get("g"),
                "b": color_info.get("b"),
...
def write_fdcf_reference_json(
    output,
    variant: str = "color01",
    nc_path: Optional[str] = None,
    active_only: bool = False,
) -> str:
    """Write FDCF reference metadata to JSON."""
...
def write_fdcf_reference_csv(
    output,
    variant: str = "color01",
    nc_path: Optional[str] = None,
    active_only: bool = False,
) -> str:
    """Write FDCF reference metadata to CSV."""
...
def sp_fdcf_fnp01_reference():
    """Return compact reference metadata for FDCF FNP01."""

    return {
        "product": "ABI-L2-FDCF",
        "fnp": "fnp01",
        "mask_category_count": len(FDCF_MASK_CATEGORIES),
        "fire_detection_codes": sorted(FDCF_FIRE_DETECTION_CODES),
        "source_pdf_url": FDCF_PUG_PDF_URL,
        "source_reference": FDCF_PUG_REFERENCE,
    }
